 ...
 */
void node_free(struct bst_node* bn){
  while(bn == NULL){
    return;
  }
  node_free(bn->right);
...
int bst_node_size(struct bst_node* bn){
  ...
  if(bn == NULL){
    ...
    return (bst_node_size(bn->left) + 1 + bst_node_size(bn->right));
  }
}
...
void bst_insert(struct bst* bst, int key, void* value) {
  ...
  if(bst->root == NULL){
    struct bst_node* bn = (struct bst_node *)malloc(sizeof(struct bst_node));
    bn->key = key;
    bn->value = value;
...
struct bst_node* remove_node(struct bst_node* bn, int key){
  ...
  //loop through the tree, store curr node and prev node
  //compare key with bn->key
  //if they match
    //check if node has no children
    //if left/right are empty
    //set node to null, free the node
    //set left/right null in prev

  //if node has one child, either one left/right is empty
  //store the child
  ...
  if(key < bn->key){
    bn->left = remove_node(bn->left, key);
  }
  else if(key > bn->key){
    bn->right = remove_node(bn->right, key);
  }
  else{
    if(bn->left == NULL){
      struct bst_node* temp = bn->right;
      free(bn);
      return temp;
    }
    else if(bn->right == NULL){
      struct bst_node* temp = bn->left;
      free(bn);
      return temp;
    }

    struct bst_node* temp = smaller_val(bn->right);
    bn->key = temp->key;
    bn->value = temp->value;
    bn->right = remove_node(bn->right, temp->key);
...
int height(struct bst_node* bn){
  if(bn == NULL){
    ...
    int l_size = height(bn->left);
    int r_size = height(bn->right);
...
int sum_exist(struct bst_node* bn, int sum){
  ...
  if(bn->key == sum && bn->left == NULL && bn->right == NULL){
    return 1;
  }
  return (sum_exist(bn->left, sum - bn->key) || sum_exist(bn->right, sum - bn->key));
...
int num_count(struct bst_node* bn, int l, int h){
  ...
  if(bn->key < l){
    return num_count(bn->right, l, h);
  }
  if(bn->key > h){
    return num_count(bn->left, l, h);
  }
  return bn->key + num_count(bn->left, l, h) + num_count(bn->right, l, h);
}

int bst_range_sum(struct bst* bst, int lower, int upper) {
  
  return num_count(bst->root, lower, upper);
}
...
struct bst_iterator* bst_iterator_create(struct bst* bst) {
  ...
  struct bst_iterator* temp = malloc(sizeof(struct bst_iterator));
  temp->stack = stack_create();
  struct bst_node* bn = bst->root;
  while(bn != NULL){
    stack_push(temp->stack, bn);
    bn = bn->left;
  }
  return temp;
}
...
int bst_iterator_has_next(struct bst_iterator* iter) {
  ...
  struct stack* s = iter->stack;
  int size = stack_isempty(s);
  if(size == 1){
    return 0;
  }
  return 1;
...
int bst_iterator_next(struct bst_iterator* iter, void** value){
  ...
  struct bst_node* bn = stack_top(iter->stack);
  bn = stack_pop(iter->stack);
  struct bst_node* temp = bn->right;
  while(temp != NULL){
